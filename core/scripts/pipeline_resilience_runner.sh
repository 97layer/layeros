...
CMD='python3 core/scripts/pipeline_resilience_smoke.py --allow-dirty-queue'
...
if python3 core/system/evidence_guard.py --check >/dev/null 2>&1; then
  if python3 core/scripts/pipeline_resilience_smoke.py --allow-dirty-queue >>"$LOG_FILE" 2>&1; then
